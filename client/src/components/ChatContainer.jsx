...
//             <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId === authUser._id ? 'justify-end' : 'justify-start'
//  }`} >
//                 {msg.image ? (
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {safeMessages.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 ${msg.senderId === authUser._id ? 'justify-end' : 'justify-start'}`}>
            {msg.image ? (
              <img src={msg.image} alt="Sent" className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8' />
            ) : (
